...
impl Operation for LocalFileInclusionOperation {
    fn apply(&self, tree: &Tree, source_code: &str) -> OperationResult {
        check_for_lfi(tree, source_code)
    }
...
fn check_for_lfi(tree: &Tree, source_code: &str) -> OperationResult {
    ...
    let query = match Query::new(
        ...
        (function_call_expression
          ...
          (#match? @function-name "(include|include_once|require|require_once)")
        )
        "#,
    ) {
        Ok(query) => query,
